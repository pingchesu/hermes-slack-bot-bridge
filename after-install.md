...
1. Read the README in your preferred language:
   - English: [`README.md`](README.md)
   - 繁體中文: [`README.zh-TW.md`](README.zh-TW.md)
2. Read the full install guide: [`docs/INSTALL.md`](docs/INSTALL.md).
3. Make sure you have the important Slack IDs:
...
4. Let Hermes' Slack adapter accept bot messages only when Hermes is mentioned:
...
5. Add the relay bot Slack **user ID** (`U...`, not `B...`) to `SLACK_ALLOWED_USERS` in `~/.hermes/.env`, if user authorization is enabled.
6. For production, set `HERMES_SLACK_BRIDGE_ALLOWED_BOT_IDS` or `HERMES_SLACK_BRIDGE_ALLOWED_APP_IDS`, plus `HERMES_SLACK_BRIDGE_HMAC_SECRET`.
7. Restart the gateway: `hermes gateway restart`.
